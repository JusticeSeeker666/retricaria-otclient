...
function EnterGame.doLogin(cast)
  ...
  if g_game.camSystemEnabled() and modules.cam_play.serverIsOnline() then
    protocolLogin = ProtocolCamLogin.create()
  else
  ...
  protocolLogin:login(SERVER_IP, SERVER_PORT, G.account, G.password, 0, G.stayLogged)
end
